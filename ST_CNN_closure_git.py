...
def train_model(zeta, model, criterion, optimizer, scheduler, n_epochs, batch_s, loader, scheduler_flag=True):
    ...
    torch_vec = torch.tensor(vec, dtype=torch.float64).to(device)
    ...
    for epoch in range(0, n_epochs+1):
        ...
        if(epoch % 10000 == 0):
            model_name = "Closure_CNN_L_{}_zeta{}_ns{}_ep{}_sin.pth".format(L, zeta, n_samples, epoch)
            torch.save({
                "epoch": epoch,
...
def evaluate_STCNN(model, device, L, zeta, load_seed, full=True):
    ...
    X_eval = torch.tensor(np.hstack([trace_DD, trace_EE, trace_DE]), dtype=torch.float64)
    
    ST      = SE + 2*zeta*SD
    y_eval = torch.tensor(np.hstack([D, E, ST]), dtype=torch.float64)
...
def generate_loader(L, zeta, load_seed, snaps, batch_s):
    ...
    X_train = torch.tensor(np.hstack([trace_DD, trace_EE, trace_DE]), dtype=torch.float64)
    
    ST      = SE + 2*zeta*SD
    y_train = torch.tensor(np.hstack([D, E, ST]), dtype=torch.float64)
...
# initialize the NN
model = reConvNetST(inp_dim=3,out_dim=5,num_nodes=16,fs=3,seed=0).to(device) #ConvAutoencoder()
